...
[MonoDetourTargets(typeof(EnemiesReturns.Enemies.LynxTribe.LynxTribeShrine))]
internal static class LynxTotem
{
    [MonoDetourHookInitialize]
    private static void Setup()
    {
        if (!ModSupport.EnemiesReturnsMod.ModIsRunning)
        {
            return;
        }
        Mdh.EnemiesReturns.Enemies.LynxTribe.LynxTribeShrine.DropItems.ILHook(DropItems);
    }


    private static void DropItems(ILManipulationInfo info)
    {
        ILWeaver w = new(info);
        ...
        .InsertBeforeCurrent(
            ...
            w.CreateDelegateCall((EnemiesReturns.Enemies.LynxTribe.LynxTribeShrine lynxTribeShrine, UniquePickup pickup, Vector3 position, Vector3 velocity, bool isDuplicated) =>
            {
                ...
                pickupInfo.SetPickupDropSource(lynxTribeShrine.gameObject);
